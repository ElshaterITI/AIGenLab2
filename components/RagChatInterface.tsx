...
const RagChatInterface: React.FC = () => {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if(fileChunks.length === 0) {
                 throw new Error("Could not find any paragraphs in the file. Please ensure it is a valid .txt file.")
            }
            setChunks(fileChunks);
            setStatus(`Chunked file into ${fileChunks.length} parts. Generating embeddings...`);
            ...
            for (let i = 0; i < fileChunks.length; i++) {
                ...
                setStatus(`Generated embedding for chunk ${i + 1}/${fileChunks.length}...`);
            }
            embeddingsRef.current = generatedEmbeddings;

            setStatus(`Ready to chat with "${selectedFile.name}".`);
            setMessages([{role: 'model', text: `I've finished reading "${selectedFile.name}". What would you like to know?`}])
        } catch (error) {
            console.error(error);
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const similarities = embeddingsRef.current.map(emb => cosineSimilarity(queryEmbedding, emb));
            ...
            const topK = 3;
            const contextChunks = sortedIndices.slice(0, topK).map(index => chunks[index]);
            ...
            <div className="p-4 border-t border-gray-700 bg-gray-800">
                <form onSubmit={handleSendMessage} className="flex items-center space-x-3">
                    <textarea
                        ...
                        placeholder={isChatDisabled ? 'Please upload a file to start chatting' : 'Ask a question about the document...'}
                        className="flex-1 p-3 bg-gray-700 rounded-lg focus:ring-2 focus:ring-purple-500 focus:outline-none resize-none"
                        rows={1}
                        disabled={isChatDisabled}
                    />
                    <button type="submit" disabled={isChatDisabled || !inputText.trim()} className="p-3 bg-purple-600 text-white rounded-full disabled:bg-gray-500 disabled:cursor-not-allowed hover:bg-purple-700 transition-colors">
